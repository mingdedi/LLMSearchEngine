...
**✅ 已完成。** 实现细节：
- 页面标题动态更新：生成中显示 `生成中：{query}`，完成后显示 `{query}`，返回时恢复默认
- 生成耗时显示：进度文字实时显示 `已生成 N 字符 · X.Xs`，完成时显示 `生成完成 · N 字符 · X.Xs`
- 修正流程也记录 startTime（供后续扩展显示）
- 其余优化项（placeholder、loading 动画、新标签页、favicon、预设按钮）已在 Step 1-6 中提前实现
